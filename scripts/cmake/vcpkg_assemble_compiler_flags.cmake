...
function(vcpkg_detect_compilers)
    ...
    if(NOT _VCPKG_CMAKE_VARS_FILE)
        z_vcpkg_get_cmake_vars(_VCPKG_CMAKE_VARS_FILE)
...
function(vcpkg_detect_compiler_flags)
    ...
    if (VCPKG_DETECTED_CMAKE_OSX_SYSROOT)
        list(APPEND _calc_CXXFLAGS -isysroot ${VCPKG_DETECTED_CMAKE_OSX_SYSROOT})
    endif ()
    ...
    set (${_cf_CXX_DEBUG} "${_calc_CXXFLAGS} ${_calc_CXXFLAGS_DEBUG} ${VCPKG_DETECTED_CMAKE_CXX_FLAGS} ${VCPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG} -I${CURRENT_INSTALLED_DIR}/include" PARENT_SCOPE)
    set (${_cf_CXX_RELEASE} "${_calc_CXXFLAGS} ${_calc_CXXFLAGS_RELEASE} ${VCPKG_DETECTED_CMAKE_CXX_FLAGS} ${VCPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE} -I${CURRENT_INSTALLED_DIR}/include" PARENT_SCOPE)
    ...
    set (${_cf_FC_DEBUG} "${_calc_FCFLAGS} ${_calc_FCFLAGS_DEBUG} ${VCPKG_DETECTED_CMAKE_Fortran_FLAGS} ${VCPKG_DETECTED_CMAKE_Fortran_FLAGS_DEBUG} -I${CURRENT_INSTALLED_DIR}/include" PARENT_SCOPE)
    set (${_cf_FC_RELEASE} "${_calc_FCFLAGS} ${_calc_FCFLAGS_RELEASE} ${VCPKG_DETECTED_CMAKE_Fortran_FLAGS} ${VCPKG_DETECTED_CMAKE_Fortran_FLAGS_RELEASE} -I${CURRENT_INSTALLED_DIR}/include" PARENT_SCOPE)

    # linker
    set(_calc_LDFLAGS)
    if (VCPKG_DETECTED_CMAKE_OSX_SYSROOT)
        list(APPEND _calc_LDFLAGS -L${VCPKG_DETECTED_CMAKE_OSX_SYSROOT}/usr/lib)
    endif ()
    set (${_cf_LD_DEBUG} "${_calc_LDFLAGS} ${VCPKG_DETECTED_CMAKE_EXE_LINKER_FLAGS} ${VCPKG_DETECTED_CMAKE_EXE_LINKER_FLAGS_DEBUG} -L${CURRENT_INSTALLED_DIR}/debug/lib" PARENT_SCOPE)
    set (${_cf_LD_RELEASE} "${_calc_LDFLAGS} ${VCPKG_DETECTED_CMAKE_EXE_LINKER_FLAGS} ${VCPKG_DETECTED_CMAKE_EXE_LINKER_FLAGS_RELEASE} -L${CURRENT_INSTALLED_DIR}/lib" PARENT_SCOPE)
endfunction()
